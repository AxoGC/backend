package main

import (
	"errors"

	"github.com/axogc/backend/utils"
	p "github.com/bestcb2333/gin-gorm-preloader/v2"
	"github.com/gin-gonic/gin"
	"gorm.io/gorm"
)

func EditPosts(cfg *HandlerConfig) (string, string, []gin.HandlerFunc) {
	return "PATCH", "/posts/:slug", []gin.HandlerFunc{
		p.Preload(
			&cfg.Config, &p.Option{Permission: p.Login, Bind: p.Uri | p.JSON}, nil,
			func(c *gin.Context, u *utils.User, r *struct {
				OldSlug  string `uri:"slug" binding:"required"`
				Slug     string `json:"slug" binding:"required,min=3"`
				Title    string `json:"title" binding:"required"`
				ForumID  uint   `json:"forumId" binding:"required"`
				Content  string `json:"content"`
				Markdown *bool  `json:"markdown"`
			}) (int, *Resp) {

				var post utils.Post
				if err := cfg.DB.Take(&post, "slug = ?", r.OldSlug).Error; errors.Is(err, gorm.ErrRecordNotFound) {
					return 404, Res("找不到对应的帖子", nil)
				} else if err != nil {
					c.Error(err)
					return 500, Res("查找帖子失败", nil)
				} else if post.UserID != u.ID && !u.Admin {
					return 403, Res("你没有权限删除该内容", nil)
				}

				if result := cfg.DB.Where(&utils.Post{Slug: r.OldSlug}).Updates(r); errors.Is(result.Error, gorm.ErrCheckConstraintViolated) {
					return 404, Res("找不到对应的论坛", nil)
				} else if errors.Is(result.Error, gorm.ErrDuplicatedKey) {
					return 409, Res("此标识或标题已被其他论坛使用", nil)
				} else if result.Error != nil {
					c.Error(result.Error)
					return 500, Res("帖子更新失败", nil)
				} else {
					return 200, Res("帖子更新成功", nil)
				}
			},
		),
	}
}
