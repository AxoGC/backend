...
func Transfer(cfg *HandlerConfig) (string, string, []gin.HandlerFunc) {
	return "POST", "/transfer", []gin.HandlerFunc{
		CheckRoleMidWare(cfg.Config, Owner), p.Preload(
			...
			func(c *gin.Context, u *utils.User, r *struct {
				...
				if user.GuildRole != Member && user.GuildRole != Admin {
				...
				if err, ok := cfg.DB.Transaction(func(tx *gorm.DB) error {
					...
					if err := tx.Save(&utils.User{ID: u.ID, GuildRole: Admin}).Error; err != nil {
						return utils.TxRes(500, Res("我的公会角色降低失败", nil))
					}

					if err := tx.Save(&utils.User{ID: user.ID, GuildRole: Owner}).Error; err != nil {
